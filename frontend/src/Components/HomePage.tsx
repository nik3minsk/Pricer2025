...
import SellerAddAndEdit, {PriceSettings} from './SellerAddAndEdit.tsx';
...
const BACK_URL = import.meta.env.VITE_BACK_URL;
        ...
        const HomePage = (): React.JSX.Element => {
            const [openedGeneralPrice, {
                open: openGeneralPrice,
                close: closeGeneralPrice
            }] = useDisclosure(false);
            const [openedSupplierPrice, {
                open: openSupplierPrice,
                close: closeSupplierPrice
            }] = useDisclosure(false);
            const [openedEGeneralPrice, {
                open: openEGeneralPrice,
                close: closeEGeneralPrice
            }] = useDisclosure(false);
            ...
            const editPrice = async () => {
                if (selectedSellerId) {
                    try {
                        ...
                        setOwnSettings(transformToPriceSettings(response.data))
                        console.log(transformToPriceSettings(response.data))
